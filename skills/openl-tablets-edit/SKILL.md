...
`openl write` は、JSON に記録された元Excel（`source_file`、`openl read` 実行時のファイル名）が同じディレクトリに存在する場合は**パッチモード**で書き込む。パッチモードはセルスタイル・書式・列幅などを保持し、変更箇所のみを差分反映する。元ファイルが見つからない場合はフルリビルド（全体再構築）にフォールバックする。どちらのモードが使われたかは実行結果に表示される。
